...
These are the simple steps I would follow to deploy the website on AWS.
...
Upload the files from the `website/` folder:

```bash
aws s3 cp website/ s3://binge-watch-online-site/ --recursive
```
...
When the website changes, sync the folder again:
